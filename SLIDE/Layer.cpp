...
//TODO: Initialize Hash Tables and add the nodes. Done by Beidi
    ...
    if (LOADWEIGHT) {
      ...
      _weights.resize(_noOfNodes * previousLayerNumOfNodes);
        ...
        std::generate(_weights.begin(), _weights.end(), [&] () { return distribution(dre); });
        std::generate(_bias.begin(), _bias.end(), [&] () { return distribution(dre); });
...
int Layer::queryActiveNodeandComputeActivations(std::vector<int*> &activenodesperlayer, float** activeValuesperlayer, std::vector<int> &lengths, int layerIndex, int inputID, const int* label, int labelsize, float Sparsity)
{
    //LSH QueryLogic
...
void Layer::saveWeights(const std::string &file) const
{
    if (_layerID==0) {
        cnpy::npz_save(file, "w_layer_0", _weights.data(), {_noOfNodes, _Nodes[0].getDim() }, "w");
        ...
        cnpy::npz_save(file, "w_layer_"+ to_string(_layerID), _weights.data(), {_noOfNodes, _Nodes[0].getDim() }, "a");
