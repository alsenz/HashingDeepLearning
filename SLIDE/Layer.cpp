...
namespace slide {
  ...
  Layer::Layer(size_t noOfNodes, int previousLayerNumOfNodes, int layerID,
    ...
    if (HashFunction == 1) {
      ...
      _MinHasher->getMap(previousLayerNumOfNodes);
    }
...
#pragma omp parallel for // num_threads(1)
  ...
  void Layer::updateTable() {
    if (HashFunction == 1) {
      ...
      _MinHasher->getMap(_previousLayerNumOfNodes);
    }
    else if (HashFunction == 4) {
  ...
  void Layer::addtoHashTable(SubVector<float> &weights, int ID) {
    ...
    if (HashFunction == 1) {
      ...
      hashes = _MinHasher->getHash(weights);
    }
    else if (HashFunction == 4) {
    ...
    if (Sparsity == 1.0) {
      ...
      if (Mode == 1) {
        ...
        if (HashFunction == 1) {
          ...
          hashes = _MinHasher->getHash(activeValuesperlayer[_layerID]);
        }
        else if (HashFunction == 4) {
