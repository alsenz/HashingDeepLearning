...
public:
  ...
  Node &getNodebyID(size_t nodeID);
	...
	int queryActiveNodeandComputeActivations(std::vector<int*> &activenodesperlayer, std::vector<float*> &activeValuesperlayer, std::vector<int> &lengths, int layerID, int inputID,  const int* label, int labelsize, float Sparsity);
