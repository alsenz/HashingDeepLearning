    ...
    for (int j = _numberOfLayers - 1; j >= 0; j--) {
      ...
      for (int k = 0; k < activeNodesPerBatch[i][j + 1].size(); k++) {
        ...
        if (j != 0) {
          ...
          node.backPropagateFirstLayer(inputIndices[i], inputValues[i], tmplr,
                                       i);
        }
      }
    }
    ...
    for (size_t m = 0; m < _hiddenlayers[l]->noOfNodes(); m++) {
      ...
      if (tmpRehash) {
        ...
        if (HashFunction == 1) {
          ...
          hashes = _hiddenlayers[l]->_dwtaHasher->getHash(local_weights);
        } else if (HashFunction == 3) {
          hashes = _hiddenlayers[l]->_MinHasher->getHashEasy(local_weights, TOPK);
